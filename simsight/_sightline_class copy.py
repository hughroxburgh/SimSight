...
class Sightline():
    ...
    def get_subsightline(self,idx,with_values=False,observed=False):
        """
        ...
        if with_values:
            source = self.observed if observed else self

            subsightline.sub_Grid = source.sub_Grid[idx]
            subsightline.sub_Compute = source.sub_Compute[idx]
            subsightline.sub_Density = source.sub_Density[idx]
            subsightline.sub_CellConditions = source.sub_CellConditions[idx]
            subsightline.sub_Halos = source.sub_Halos[idx]
            subsightline.sub_HaloAssignment = source.sub_HaloAssignment[idx]
            subsightline.sub_BoxRedshifts = source.sub_BoxRedshifts[idx]
            subsightline.sub_Snapshots = source.sub_Snapshots[idx]
    ...
    def plot_compute(self,idx=None,data='compute',with_model=False,return_data=False,logspace=True,mode='normal'):
        ...
        if data not in ('compute', 'density'):
        ...
        if with_model:
            ...
            if max_idx > 0 and self.observed is not None:
                model_lengths = [self.observed.sub_Grid[idx]] if idx is not None else self.observed.sub_Grid
                model_computed = [getattr(self.observed, attr)[idx]] if idx is not None else getattr(self.observed, attr)
    ...
    def subsightline_reached(self,grid=True,halos=False,assigned=False,observed=False,modelled=False):
        ...
        if observed:
            if self.observed is not None:
                checks.append(next((i for i, h in enumerate(self.observed.sub_Halos) if h == [] or any('GalaxyVisible' not in d for d in h)),
                    self.num_sub_sightlines))
                # checks.append(next((i for i, h in enumerate(self.observed.sub_Halos) if h == []), self.num_sub_sightlines))
            else:
                checks.append(0)
        if modelled:
            if self.observed is not None:
                checks.append(next((i for i, h in enumerate(self.observed.sub_Grid)  if len(h) == 0), self.num_sub_sightlines))
            else:
                checks.append(0)
    ...
    def extract_compute(self, cosmo, redshift=None, environment='total',modelled=False):
        ...
        grid = getattr(self.observed, 'sub_Grid') if modelled else getattr(self, 'sub_Grid')
        compute = getattr(self.observed, 'sub_Compute') if modelled else getattr(self, 'sub_Compute')
        conditions = getattr(self.observed, 'sub_CellConditions') if modelled else getattr(self, 'sub_CellConditions')
    ...
    def halo_info(self,modelled=False,with_compute=True):
        ...
        source = self.observed if modelled else self
    ...
    def _initialise_observed(self,with_halos=False):

        obs = Sightline(origin=self.origin,
                    direction_vector=self.direction_vector,
                    length=self.length,
                    parent_sightline=False)
    
        obs.num_sub_sightlines = self.num_sub_sightlines
        obs.sub_Origins = self.sub_Origins
        obs.sub_Lengths = self.sub_Lengths
        obs.sub_BoxRedshifts = self.sub_BoxRedshifts
        obs.sub_Snapshots = self.sub_Snapshots

        # These get populated by the inference pipeline
        if with_halos:
            obs.sub_Halos = deepcopy(self.sub_Halos)
            for sh in obs.sub_Halos:
                if h == [None]:
                    h = []
        else:
            obs.sub_Halos = [[] for _ in range(self.num_sub_sightlines)]          # only LSST-visible halos
        obs.sub_Density = [[] for _ in range(self.num_sub_sightlines)]
        obs.sub_Compute = [[] for _ in range(self.num_sub_sightlines)]        # inferred DM
        obs.sub_Grid = [[] for _ in range(self.num_sub_sightlines)]           # can mirror original
        obs.sub_CellConditions = [[] for _ in range(self.num_sub_sightlines)]
        obs.sub_HaloAssignment = [[] for _ in range(self.num_sub_sightlines)]
            ...
            if self.sub_Snapshots[i] == galaxyfinder.snapshot: 
                if self.sub_Halos[i] != [None]:
                    for halo in self.sub_Halos[i]:
                        ...
                        self.observed.sub_Halos[i].append(halo_info)

                else:
                    self.observed.sub_Halos[i] = [None]
    ...
    def infer_halos(self, inference, filters):
        ...
        for i in range(num_sub_sightlines):
            ...
            if self.observed.sub_Halos[i] != [None]:
                for halo in self.observed.sub_Halos[i]:
    ...
    def model_sightline(self, inference, halo_params='inferred', igm_background='smooth_truth', 
                        observed=True, density_smooth_kernel=1000,filters=None,verbose=True):

        num_sub_sightlines = self.subsightline_reached(grid=True,observed=observed)
        if num_sub_sightlines == 0:
            if verbose:
                print("Halos have not been observed! Switching 'observed' to False")
            observed = False
            num_sub_sightlines = self.subsightline_reached(grid=True)
            self._initialise_observed(with_halos=True)
            if halo_params == 'inferred':
                if verbose:
                    print("Halo properties have not been inferred! Switching 'halo_params' to 'truth'")
                halo_params = 'truth'

        elif halo_params == 'inferred':
            self.infer_halos(inference,filters)

        elif halo_params not in ['truth',False]:
            raise ValueError("halo_params must be 'inferred' , 'truth', or set to False.")

        for i in range(num_sub_sightlines):
            subsightline = self.get_subsightline(i,with_values=True,observed=observed)
                    
            grid,density,compute,conditions,assign = inference.infer_dm(subsightline,igm_background,halo_params,density_smooth_kernel)
            
            self.observed.sub_Grid[i] = grid
            self.observed.sub_Density[i] = density
            self.observed.sub_Compute[i] = compute
            self.observed.sub_CellConditions[i] = conditions
            self.observed.sub_HaloAssignment[i] = assign
